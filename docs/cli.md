...
This page covers the command-line interface. Start with
[the quick start](../README.md) for installation, model files, and model config
YAMLs.
...
- output files and JSON sidecars are not overwritten unless `--force` is passed.

## Model Inference

Use `omniem infer` for a single image or volume.
...
The model inference CLI reads image files, applies input scaling, applies the
selected normalization, then runs the model. Model configs use `[0, 1]`-domain
`mean`/`std`, so the default path scales integer EM images into `[0, 1]` before
the fixed model normalization runs.

#### Input Scaling (`--scale`)

`--scale` brings the input into `[0, 1]`. It defaults to `dtype`, so integer
images "just work" without an explicit divisor.
...
Signed-integer inputs divide into `[-1, 1]` and trip a warning (warn-only). If
...
After scaling, `omniem` normalizes. The norm source is independent of the scale
axis:

| `--norm` | meaning |
|---|---|
| `model` (default) | model config `mean`/`std` |
...
Model inference CLI accepts simple grayscale arrays:

- 2D input is interpreted as `yx`;
- 3D input is interpreted as `zyx`.

For richer layouts such as `cyx`, `czyx`, or `bcyx`, use the encoder features
CLI with `--axes`. If your file has color channels, declare them with `--axes`
and use `--color-to-gray` when the channels should be averaged.

#### Non-Conforming XY Shapes

`omniemv1` expects square XY input whose side is a multiple of the model stride
...
| `pad` | pad bottom/right, run the model, crop back to original XY |
| `resize` | resize XY before inference, then resize output back |

`pad` is lossless in image geometry. `resize` is useful when you accept
interpolation.
...
| `-m`, `--model` | model config YAML |
| `--weights` | merged whole-model state dict |
...
- `out/mito_labels.tif.json`: a sidecar with config, weights, scaling, norm,
  conform mode, and output metadata.

No `--scale` is needed: a `uint8` tile uses the default `--scale dtype` (divide
by 255 into `[0,1]`), which matches these `[0,1]`-domain model configs.
...
The model sees a conformed square input, but the written output is returned to
the original XY shape. The sidecar records both original and conformed XY.
...
- The flag resizes the **input** XY by `F` (bicubic), then runs the
  shape-preserving model. The example input `2d_MitoEM_H_0_0_0.tif` is `224×224`, so
...
| `--weights PATH` | raw `vit.*` encoder checkpoint |
...
single **merged** whole-model `.pt` and the **split** backbone + head pair. Both
...
| Argument | Meaning |
|---|---|
| `-m`, `--model` | model config YAML (**required** — see note below) |
...
`-m CONFIG` is required because the backbone/head boundary is the model's **derived
encoder prefix** — it is read from the net the config builds, not assumed to be
`vit.*`. The same config that loads a model is the one that splits or merges its
weights.
